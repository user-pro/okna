...
class TabList {
   constructor(buttonsContainer, tabs) {
      ...
      this.buttonsContainer.addEventListener('click', event => {
         const index = event.target.closest('.gb-other-questions__tabs-button').dataset.value;
         document.querySelector(`.gb-other-questions__tabs-button.active`).classList.remove(`active`);

         event.target.closest('.gb-other-questions__tabs-button').classList.add(`active`);
         this.openTab(index);
      });
   }
...
const tabButtonsContainer = document.querySelector('.gb-other-questions__tabs-buttons');
const tabsContainer = document.querySelector('.gb-other-questions__tabs');
if (tabButtonsContainer) {

   new TabList(tabButtonsContainer, tabsContainer);
}

function rewriteSliderArrows(prevArr, nextArr, slider) {
...
if (gallerySlider.classNames.length) {
   rewriteSliderArrows('.gallery__button-prev', '.gallery__button-next', gallerySlider);
}
...
const topCalcSlider = document.getElementById('calc-top-slider');

if (topCalcSlider) {
   noUiSlider.create(topCalcSlider, {
      start: 600,
      step: 1,
      connect: 'lower',
      range: {
   ...
   const directionFieldTop = document.getElementById('mp-calculator-image__top-slider-field');
   topCalcSlider.noUiSlider.on('update', function (values, handle) {
      directionFieldTop.innerHTML = Math.round(values[handle]);
   });
   topCalcSlider.noUiSlider.on('update', function (values, handle) {
      directionFieldTop.value = Math.round(values[handle]);
   });
   directionFieldTop.addEventListener('change', function () {
      topCalcSlider.noUiSlider.set(this.value);
   });
}


const rightCalcSlider = document.getElementById('calc-right-slider');
if (rightCalcSlider) {
   noUiSlider.create(rightCalcSlider, {
      start: 1200,
      connect: 'lower',
      step: 1,
      range: {
   ...
   const directionFieldRight = document.getElementById('mp-calculator-image__right-slider-field');

   rightCalcSlider.noUiSlider.on('update', function (values, handle) {
      directionFieldRight.innerHTML = Math.round(values[handle]);
   });

   rightCalcSlider.noUiSlider.on('update', function (values, handle) {
      directionFieldRight.value = Math.round(values[handle]);
   });
   directionFieldRight.addEventListener('change', function () {
      rightCalcSlider.noUiSlider.set(this.value);
   });
}
...
const calculatorSashButtons = document.querySelector(`.calculator-sash__buttons`);
// const calculatorWIndows = document.querySelectorAll(`.mp-claculator-window`);


const windowOuterBlock = document.querySelector(`.mp-calculator-image__window`);

if (document.querySelector(`.active-window-image`)) {

   const activeWindowWidth = document.querySelector(`.active-window-image`).clientWidth;
   const activeWindowHeight = document.querySelector(`.active-window-image`).clientHeight;



   const rightSlider = document.querySelector(`.mp-calculator-image__right-slider-wrapper`);
   // const topSlider = document.querySelector(`.mp-calculator-image__top-slider-wrapper`);




   if (windowOuterBlock) {
      windowOuterBlock.style.width = activeWindowWidth + 'px';
   }
   if (rightSlider) {
      rightSlider.style.height = activeWindowHeight + 'px';
   }
   // if (topSlider) {
   //    topSlider.style.width = activeWindowWidth + 'px';
   // }
}
...
windowStates = {
   1: {
      text: 'Глухая створка',
      img: `url(${siteUrl}/assets/img/leaf-x.png);`
   },
   2: {
      text: 'Повортная влево',
      img: `url(${siteUrl}/assets/img/leaf-l.png);`
   },
   3: {
      text: 'Повортно/откидная левая',
      img: `url(${siteUrl}/assets/img/leaf-lu.png);`
   },
   4: {
      text: 'Повортная вправо',
      img: `url(${siteUrl}/assets/img/leaf-r.png);`
   },
   5: {
      text: 'Повортная/откидная правая',
      img: `url(${siteUrl}/assets/img/leaf-ru.png);`
   },
}

function changeWindowState(elem) {
   let currentState = elem.getAttribute(`data-window-sate`);
   if (currentState < 5) {
      currentState++;
      elem.setAttribute(`data-window-sate`, currentState);

   } else if (currentState == 5) {
      elem.setAttribute(`data-window-sate`, 1);

   }
};
function changeWindowData(elem) {
   let currentState = elem.getAttribute(`data-window-sate`);
   elem.querySelector(`.mp-calculator-image-outer-window__text`).textContent = windowStates[`${currentState}`].text;
   let imageUrl = windowStates[`${currentState}`].img;
   elem.style.cssText = `
      color: #000;
...
const calcWindows = document.querySelectorAll(`.mp-claculator-window`);

if (calcWindows) {
   calcWindows.forEach(item => {
      item.querySelectorAll(`.mp-calculator-image-outer-window__background`).forEach(obj => {
         obj.addEventListener('click', () => {
            changeWindowState(obj);
            changeWindowData(obj);
         });
      });
   });
}

const calculatorSashBtn = document.querySelectorAll('.calculator-sash__btn');

if (calculatorSashBtn) {
   calculatorSashBtn.forEach(item => {
      item.addEventListener('click', () => {
         document.querySelector(`.calculator-sash__btn.active`).classList.remove(`active`);
         item.classList.add(`active`);
         let sashCount = item.getAttribute('data-sash-count');
         calcWindows.forEach(wind => {
            wind.style.display = 'none';
            wind.classList.remove(`active-window-image`);
            if (wind.getAttribute('data-sash-count') == sashCount) {
               wind.style.display = 'flex';
               wind.classList.add(`active-window-image`);
            }
         });
         const mpClaculatorWindowWidth = document.querySelector(`.mp-claculator-window.active-window-image`).clientWidth;

         if (windowOuterBlock) {
            windowOuterBlock.style.width = mpClaculatorWindowWidth + 'px';
...
// const applicationForms = document.querySelectorAll('.form-send');
...
function mailTest(value) {
   phoneArr = value.split(''),
      phoneSymbols = phoneArr.filter(i => i !== ' ').join(''),
      re1 = /^\+\d\s\(\d\d\d\)\s\d\d\d\-\d\d\-\d\d$/,
      re2 = /^\d\d\d\d\d\d\d\d\d\d\d$/,
      re3 = /^\+\d\d\d\d\d\d\d\d\d\d\d$/,
      re4 = /^\+\d\s\d\d\d\s\d\d\d\s\d\d\s\d\d$/;

   if (phoneSymbols.length >= 11) {
      if (re1.test(value) || re2.test(value) || re3.test(value) || re4.test(value)) {
         return true;
      } else if (re2.test(phoneSymbols)) {
         return true;
      } else {
         return false;
      }
...
// async function sendMail(form, dataList) {
//    let response = await fetch(`${siteUrl}/sendmail.php`, {
//       method: 'POST',
//       body: dataList
//    });
//    if (response.ok) {
...
//       alert("отправлено" + JSON.stringify(result));
//    } else {
//       alert('Ошибка');
//    }
// }
...
; (function () {
   var forms = document.querySelectorAll('.js-send-mail-form');
   ...
   var formSend = function (form) {
      ...
      let phone = form.querySelector('input[name="user_phone"]').value,
         name = '',
      ...
      if (isName && name || !isName) {
         ...
         if (mailTest(phone)) {
            ...
            var url = `${ajaxUrl}?action=send_email`;
...
; (function () {
   ...
   var ymapLoad = function () {
      let locationMapBlock = document.querySelector('.location__map');
      let script = document.createElement('script');
      script.src = 'https://api-maps.yandex.ru/services/constructor/1.0/js/?um=constructor%3Ad6ceb0968004c4708e08d639c95e8ca9afaf5a9d6c06bb58c0c6c4ee5c37d06c&amp;width=100%25&amp;height=365&amp;lang=ru_RU&amp;scroll=true';
      locationMapBlock.appendChild(script);
   };

   var checkYmapInit = function () {
      var sectionLocationTop = sectionLocation.getBoundingClientRect().top;
      var scrollTop = window.pageYOffset;
      var sectionLocationOffsetTop = scrollTop + sectionLocationTop;

      if (scrollTop + window.innerHeight > sectionLocationOffsetTop) {
         ymapLoad();
